...
void listar()
{
	...
	{
		printf("%-25s %-10s %-35s %-10s %-35s %-10s", "NOME", "TELEFONE", "LOGRADOURO", "NUMERO", "CIDADE", "ESTADO");
		fread(&end,sizeof(endereco),1,meuArquivo);
		while(!feof(meuArquivo))
		{
			if (strcmp(end.nome, "0") != 0)
				printf("\n%-25s %-10d %-35s %-10d %-35s %-10s", end.nome, end.telefone, end.logradouro, end.numero, end.cidade, end.estado);
			fread(&end,sizeof(endereco),1,meuArquivo);
		}
		fclose(meuArquivo);
	}
